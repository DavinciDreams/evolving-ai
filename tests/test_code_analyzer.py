...
async def test_code_analyzer_pipeline():
    ...
    try:
        ...
        print("\n✅ Analysis completed!")
        print(
            f"Improvement Potential: {analysis_result.get('improvement_potential', 0):.2f}"
        )
        ...
        print(
            f"  Average Complexity: {complexity_metrics.get('average_complexity', 0):.2f}"
        )
        ...
        if high_complexity:
            ...
            for func in high_complexity[:5]:  # Show top 5
                print(
                    f"  - {func.get('function', 'Unknown')} in {func.get('module', 'Unknown')}: {func.get('complexity', 0)}"
                )
...
async def test_specific_module_analysis():
    ...
    try:
        ...
        for cls in classes:
            ...
            inheritance_info = (
                f" extends {', '.join(inheritance)}" if inheritance else ""
            )
