...
async def test_api_endpoints():
    ...
    async with httpx.AsyncClient() as client:
        ...
        try:
            ...
            response = await client.post(
                f"{base_url}/analyze", json=analysis_request, timeout=30.0
            )
            print(f"Status: {response.status_code}")
            if response.status_code == 200:
                ...
                print(
                    f"Improvement potential: {analysis_data['improvement_potential']:.2f}"
                )
                print(
                    f"Opportunities found: {len(analysis_data['improvement_opportunities'])}"
                )
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                for i, entry in enumerate(history, 1):
                    print(
                        f"  {i}. {entry.get('timestamp', 'Unknown')} - Potential: {entry.get('improvement_potential', 0):.2f}"
                    )
            else:
                print(f"Error: {response.text}")
        except Exception as e:
