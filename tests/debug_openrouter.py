...
async def debug_openrouter():
    ...
    print(
        f"Config OpenRouter key: {config.openrouter_api_key[:10] if config.openrouter_api_key else 'None'}..."
    )
    ...
    try:
        ...
        response = await interface.generate_response(
            prompt="Hello! Please just say 'Working!'", temperature=0.1, max_tokens=20
        )
        ...
        payload = {
            "model": working_model,
            "messages": [
                {"role": "user", "content": "Hello! Please just say 'Working!'"}
            ],
            "temperature": 0.1,
            "max_tokens": 20,
        }
        ...
        try:
            async with httpx.AsyncClient(timeout=60.0) as client:
                ...
                if response.status_code == 200:
                    data = response.json()
                    print(
                        f"✅ Direct HTTP SUCCESS: {data['choices'][0]['message']['content']}"
                    )
                else:
                    print(
                        f"❌ Direct HTTP ERROR: {response.status_code} - {response.text}"
                    )
