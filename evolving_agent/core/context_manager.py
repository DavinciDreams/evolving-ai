...
class ContextManager:
        ...
        try:
            ...
            for ctx_type in context_types:
                ...
                try:
                    ...
                    context_queries.append(ContextQuery(
                        query=search_query.strip(),
                        context_type=ctx_type,
                        priority=self._calculate_context_priority(ctx_type, main_query),
                        timestamp=datetime.now(),
                        metadata={"main_query": main_query}
                    ))
                    
                except Exception as e:
                    logger.warning(f"Failed to generate context query for {ctx_type}: {e}")
    ...
    def _filter_and_rank_memories(
        ...
        try:
            ...
            for memory, similarity in memories:
                ...
                recency_score = max(0, 1 - (time_diff.total_seconds() / (30 * 24 * 3600)))  # 30 days decay
    ...
    async def _organize_contexts(
        self,
        all_contexts: Dict[str, List[Tuple[MemoryEntry, float]]],
        main_query: str
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            for context_type, memories in all_contexts.items():
                ...
                if len(context_items) > 5:
                    summary = await self._summarize_context_items(context_items, context_type)
