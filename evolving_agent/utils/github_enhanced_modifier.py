...
class GitHubEnabledSelfModifier:
        ...
        try:
            ...
            if create_pr and self.auto_pr_enabled and validated_improvements:
                if self.github_integration.repository:
                    ...
                    logger.warning(
                        "GitHub repository not available, skipping PR creation"
                    )
    ...
    async def _validate_improvements(
        ...
        for improvement in improvements:
            try:
                # Basic validation
                if not improvement.get("description") or not improvement.get(
                    "priority"
                ):
                    continue
                ...
                if improvement_type == "function_refactor":
                    ...
                    if (
                        improvement.get("current_complexity", 0) > 10
                    ):  # Only refactor truly complex functions
        ...
        logger.info(
            f"Validated {len(validated)} out of {len(improvements)} improvements"
        )
        return validated
