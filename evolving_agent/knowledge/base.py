...
class KnowledgeBase:
    ...
    def _calculate_relevance(self, query: str, entry: KnowledgeEntry) -> float:
        ...
        try:
            ...
            # Tag matches
            query_words_in_tags = any(word in ' '.join(entry.tags).lower() for word in query_words)
            ...
            score += min(entry.access_count * 0.01, 0.1)  # Small boost for frequently accessed
    ...
    async def update_knowledge(self, knowledge_id: str, updated_entry: KnowledgeEntry) -> bool:
    ...
    async def add_category(self, name: str, description: str, tags: List[str], priority: float = 0.5):
